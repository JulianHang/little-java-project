...
public class MaximumBinaryHeap<T extends Comparable<T>> {
    ...
    /**
     * 下浮节点以满足二叉堆的性质
     * 思路：不管移除的是哪个节点，拿末尾的节点是最少成本的，因为拿末尾的节点的值去计算是否覆盖移除节点的值来使其还是一颗完全二叉树，所以只要二叉堆性质就可以了
     ...
     *
     * 2. 删除的节点有子节点，针对最大堆，先比较两个子节点的值的大小，获取值最大的节点，如下步骤：
     *       21. 在将值最大的节点与末尾的节点进行比较（若只有一个子节点的话，那么只能是左子节点，就直接比较大小）
     *       22. 若是末尾的节点更大的话，那么直接覆盖移除的节点的值即可，同时将末尾位置设置成null，最后退出
     *       23. 若是其子子节点更大的话，那么用值最大的节点覆盖到移除的节点的值，此时值最大的节点的位置就空出来了（相当于此时它被移除了），那么此时重复步骤12
     *
     * 假设一个节点的索引为：N，其父节点的索引为：(N - 1)/2，左子节点的索引为：2N + 1，其右子节点的索引为：2N + 2
     * 该算法参考自ScheduledThreadPoolExecutor#DelayWorkQueue
